...
-- >,<,>=,<=,=, !=,<>,^= (서브쿼리)
  ...
  --SMITH 사원의 급요보다 많이받는 직원의
  -- 사번, 이름, 부서명, 급여를 조회하세요
SELECT A.EMPNO 사번, A.ENAME 이름, B.DNAME 부서, A.SAL 급여 
FROM EMP A
...
FROM EMP A
LEFT JOIN DEPT B ON A.DEPTNO = B.DEPTNO 
WHERE A.EMPNO IN(SELECT DISTINCT MGR -- IN != NOT IN
				FROM EMP WHERE MGR IS NOT NULL AND MGR != 0)
...
FROM EMP A
LEFT JOIN DEPT B ON A.DEPTNO = B.DEPTNO 
--WHERE A.EMPNO IN (7839, 7782, 7698, 7902, 7566, 7788) -- IN = OR / MGR의 값을 안에 입력한 모습
WHERE A.EMPNO NOT IN(SELECT DISTINCT MGR -- IN != NOT IN
				FROM EMP WHERE MGR IS NOT NULL AND MGR != 0);
...
SELECT * FROM EMP
WHERE (JOB, TRUNC(SAL, 0 )) IN (SELECT JOB , TRUNC(AVG(SAL), 0)
								FROM EMP
								GROUP BY JOB 
...
SELECT
	...
	, ENAME
	, SAL
FROM EMP; -- DESC를 걸면 순서가 엉킴 / ROWNUM 수행 이전에 진행해야함
...
FROM (
	...
	A.*
	FROM EMP A
	ORDER BY SAL DESC -- ROWNUM을 사용시 ORDER BY 로 인해 ROWNUM이 흐트러 지기때문에 우선 진행
)
WHERE ROWNUM <= 5;
...
FROM (
	...
	FROM EMP A LEFT JOIN DEPT ON A.DEPTNO = B.DEPTNO
	GROUP BY A.DEPTNO
