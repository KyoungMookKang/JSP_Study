...
-- INSERT INTO MEMBER values('100','100','park','011'); -- 값을 하나 더 추가하면 에러가 발생(밸류값이 많다는 에러)

INSERT INTO MEMBER (ID, PW, NAME) VALUES ('100', '100', 'park'); -- 넣고자 하는 컬럼 순서 중요

INSERT INTO MEMBER (ID, PW, NAME, COMPANY) VALUSE ('100', '100', 'park', 'LG'); -- 정의되지 않은 컬럼에 넣고자 할 때 에러발생
commit;
...
SELECT * FROM MEMBER ORDER BY ID; -- 오름차순 < 기본값 = ASC != DESC
...
INSERT INTO USER_FOREIGNKEY
    (USER_NO, USER_ID , USER_PWD , USER_NAME , GENDER , PHONE , EMAIL , GRADE_CODE)
    VALUES
    (4, 'user04', 'pass04', '유재석', '남', '010-1234-5678', 'yu@naver.or.kr' ,50); --SQL Error [2291] [23000]: ORA-02291: integrity constraint (SCOTT.FK_GRADE_CODE) violated - parent key not found
...
-- ON DELETE RESTRICTED < 기본적으로 설정되어 있음
...
INSERT INTO USER_FOREIGNKEY2
    (USER_NO, USER_ID , USER_PWD , USER_NAME , GENDER , PHONE , EMAIL , GRADE_CODE)
    VALUES
    (4, 'user04', 'pass04', '유재석', '남', '010-1234-5678', 'yu@naver.or.kr' ,NULL);
...
INSERT INTO MEMBER4 VALUES('100','100','park','011');
COMMIT;
...
ALTER TABLE MEMBER5 MODIFY NAME NULL; -- 이미 NOT NULL 이면 NULL로 변경한다.
...
-- CASCADE CONSTRAINT : 제약조건까지 한번에 삭제
DROP TABLE TEST11 CASCADE CONSTRAINT;
DROP TABLE TEST22; -- 테이블만 삭제시
